...
$host = getenv('DOCKER_ENV') === '1' ? 'db' : '127.0.0.1';
// print_r($host);
try {
    try {
        $pdo = new PDO(
            "mysql:host=$host;port=3306;dbname=mydb;charset=utf8mb4",
            "root",
            "rootpass"
        );

        // $pdo = new PDO("mysql:host=db;dbname=mydb;charset=utf8mb4", "root", "rootpass");


        //  $pdo = new PDO(
        //     "mysql:host=127.0.0.1;port=3306;dbname=mydb;charset=utf8mb4",        //PHP on host machine → host=127.0.0.1 (or localhost)
        //     "root",
        //     "rootpass"
        // );
        error_log("\n login success at: " . date("l dS M Y h:i:s"), 3, __DIR__ . "/errors.log");
    ...
    $smt = $pdo->prepare("INSERT INTO users(`name`,`email`,`age`) VALUES('ABC','ok@gmail.com',20)");
    if ($smt->execute()) {
